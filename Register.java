...
public class Register {
    ...
    void putOnRegister(String input) {//레지스터에는 001과 같은 레지스터의 위치값
        //todo 문자열로 들어온 input을 다시 배열로 변환해주어야 한다.
        ...
        switch (inputArr[0]) {
            case "0001"://LOAD_R     0001--dst.Reg--base.Reg--000--offset.Reg
                ...
                memoryVal = memory.getMemoryMapVal(memoryAddress);//메모리 밸류가 string형
                if (memoryVal == null) {//레지스터로 들어오는 값이 string이므로 비었을때 null이 반환되므로
                    memoryVal = Integer.toString(0);//정수 0으로 넣어준다 (레지스터는 정수만 받을 수 있다)
                }
                registerMap.put(inputArr[1], Integer.parseInt(memoryVal));//dst.Reg에 정수형으로 올림
                break;
            case "0010"://LOAD_V     0010--dst.Reg--base.Reg--1--offset.value(5bit)
                ...
                memoryVal = memory.getMemoryMapVal(memoryAddress);
                if (memoryVal == null) {
                    memoryVal = Integer.toString(0);
                }
                registerMap.put(inputArr[1], Integer.parseInt(memoryVal));//dst.Reg에 정수형으로 올림
                break;
            case "0011"://STORE_R
                memoryAddress = alu.add(registerMap.get(inputArr[2]), registerMap.get(inputArr[4]));//base.Reg + offset.Reg
                registerVal = registerMap.get(inputArr[1]);
                memory.putOnMemory(Integer.toString(memoryAddress), Integer.toString(registerVal));//?????
                break;
            case "0100"://STORE_V
                memoryAddress = alu.add(registerMap.get(inputArr[2]), Integer.parseInt(inputArr[4], 2));//base.Reg + offset.Reg
                registerVal = registerMap.get(inputArr[1]);
